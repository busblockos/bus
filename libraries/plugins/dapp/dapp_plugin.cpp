...
namespace sigmaengine { namespace dapp {
   namespace detail {
      ...
      void dapp_plugin_impl::aggregate_trx_fee_vote ( sigmaengine::chain::database& _db ) {
         ...
         //ilog( "vote_idx.size() = ${s}", ( "s", vote_idx.size() ) );
         while( vote_itr != vote_idx.end() ) {

            //ilog( "vote_itr->last_update + SIGMAENGINE_MAX_FEED_AGE_SECONDS = ${s}", ( "s", (vote_itr->last_update + SIGMAENGINE_MAX_FEED_AGE_SECONDS) ) );
      ...
      void dapp_plugin_impl::on_apply_block( const signed_block& b ) {
         ...
         //ilog("current : ${n} VS ${a}", ("n", now)("a", (_dgp.last_dapp_voting_aggregation_time + fc::hours(1))));
